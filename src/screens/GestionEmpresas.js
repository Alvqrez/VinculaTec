...
const SECTOR_ICON = {
  Tecnología:"cpu", Manufactura:"tool", Software:"code", Construcción:"home",
  Farmacéutica:"activity", Automotriz:"truck", Educación:"book-open",
};
...
export default function GestionEmpresas() {
  ...
  const STATUS_STYLE = {
    Activa:{ color:C.green,bg:C.greenLight }, "Por Vencer":{ color:C.amber,bg:C.amberLight },
    Nueva:{ color:C.blue,bg:C.blueLight },    Inactiva:{ color:C.red,bg:C.redLight },
  };
  ...
  // ref para medir posición del botón Filtrar
  ...
  const activeFilterCount = [
    filters.sector !== "Todos", filters.status !== "Todos", filters.ciudad !== "Todas",
  ].filter(Boolean).length;

  // ── FIX: Posiciona el dropdown via measureInWindow, luego lo muestra en Modal.
  // Así escapa del stacking-context del ScrollView anidado y siempre queda encima.
  const openFilter = () => {
  ...
  const openEdit = (co) => {
    setForm({
      name: co.name, sector: co.sector, ciudad: co.ciudad, convenio: co.convenio,
      contactoNombre: co.contactoNombre || "", contactoEmail: co.contactoEmail || "",
      contactoTel: co.contactoTel || "", status: co.status,
    });
    setEditId(co.id);
    setModal(true);
  };
  ...
  const saveCompany = async () => {
    ...
    const payload = {
      ...form,
      convenio: form.convenio ? new Date(form.convenio).toISOString().split("T")[0] : null,
    };
    if (editId) {
      const res = await apiClient.put(`/api/jefe/empresas/${editId}`, payload);
      if (res.ok) { await reloadEmpresas(); setModal(false); showToast("Empresa actualizada"); }
      else showToast(res.body?.mensaje || "Error al actualizar", "error");
    } else {
      const res = await apiClient.post("/api/jefe/empresas", payload);
      if (res.ok) { await reloadEmpresas(); setModal(false); showToast("Empresa guardada"); }
  ...
  return (
    <View style={{ flex:1, backgroundColor:C.bg }}>
        ...
        <Card style={{ padding:0, overflow:"hidden" }}>
          <View style={{ padding:18, borderBottomWidth:1, borderBottomColor:C.border }}>
            <Row style={{ justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              ...
              {/* Botón Filtrar — se mide para posicionar el Modal */}
              <TouchableOpacity
                ref={filterBtnRef}
            ...
            <Row style={{ alignItems:"center", backgroundColor:C.bg, borderRadius:9, borderWidth:1, borderColor:C.border, paddingHorizontal:12, gap:8 }}>
              ...
              <TextInput
                value={search} onChangeText={setSearch} placeholder="Buscar empresa, sector o ciudad..."
          ...
          {filtered.length === 0 ? (
            ...
            // Mostrar solo la fecha (YYYY-MM-DD) si el convenio viene en formato ISO
            const convenioDisplay = co.convenio
              ? (String(co.convenio).length > 10 ? String(co.convenio).slice(0,10) : co.convenio)
              : "—";
            return (
              <View key={co.id} style={{ paddingHorizontal:18, paddingVertical:13, borderTopWidth:1, borderTopColor:C.borderLight, backgroundColor:C.bg }}>
                <Row style={{ alignItems:"center" }}>
                  ...
                  <Text style={{ flex:1.5, fontSize:12, color:C.textSub, textAlign:"center" }}>{convenioDisplay}</Text>
                  <View style={{ flex:1, alignItems:"center" }}>
                    <Badge text={co.status} color={st.color} bg={st.bg} />
                  </View>
                  <Row style={{ flex:0.8, justifyContent:"flex-end", gap:8 }}>
                    <TouchableOpacity onPress={()=>openEdit(co)}><Feather name="edit-2" size={14} color={C.textMuted} /></TouchableOpacity>
                    <TouchableOpacity onPress={()=>deleteCompany(co.id)}><Feather name="trash-2" size={14} color={C.red} /></TouchableOpacity>
                  </Row>
                </Row>
              </View>
            );
      ...
      {/* ── Modal: Panel de filtros flotante (escapa del stacking-context del ScrollView) ── */}
        ...
        <Pressable style={{ flex:1 }} onPress={() => setFilter(false)}>
          {filterPos && (
            ...
            >
              ...
              {[["Sector","sector",SECTORES],["Estado","status",ESTADOS],["Ciudad","ciudad",CIUDADES]].map(([label, key, opts]) => (
                    ...
                    {opts.map((opt) => (
                      <TouchableOpacity
                        ...
                        style={{
                          ...
                          backgroundColor: filters[key]===opt ? C.tealLight : "white",
                        }}
                      >
      ...
      <Modal visible={showModal} transparent animationType="fade">
        <Pressable style={{ flex:1, backgroundColor:"rgba(0,0,0,0.45)", justifyContent:"center", alignItems:"center" }} onPress={()=>setModal(false)}>
          <Pressable
            style={{ width:520, backgroundColor:C.card, borderRadius:18, padding:28, borderWidth:1, borderColor:C.border, shadowColor:"#000", shadowOpacity:0.15, shadowRadius:20, shadowOffset:{width:0,height:8} }}
            onPress={()=>{}}
          >
            <Row style={{ justifyContent:"space-between", alignItems:"center", marginBottom:22 }}>
            ...
            <ScrollView style={{ maxHeight:440 }} showsVerticalScrollIndicator={false}>
              ...
              <View style={{ marginTop:12 }}>
                <Text style={{ fontSize:11, fontWeight:"700", color:C.textMuted, textTransform:"uppercase", letterSpacing:0.5, marginBottom:6 }}>Correo</Text>
