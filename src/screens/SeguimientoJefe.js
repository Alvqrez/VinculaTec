...
export default function SeguimientoJefe() {
  ...
  // ── Cargar datos desde el endpoint propio del Jefe ───────────────────────
  useEffect(() => {
    let activo = true;
    setLoading(true);
    setError(null);
    ...
    apiClient
      ...
      .then((res) => {
        ...
        if (res.ok && res.body?.ok) {
          ...
          setError(res.body?.mensaje || "Error al cargar el seguimiento");
        }
      })
      .catch(() => {
        if (activo) setError("Error de conexión con el servidor");
      })
      .finally(() => {
  ...
  const residentesFiltrados = useMemo(() => {
    ...
    return residentes.filter((r) => {
      if (filtroTipo === "residente")
        return (r.nombre || "").toLowerCase().includes(q);
      if (filtroTipo === "proyecto")
        return (r.proyecto || "").toLowerCase().includes(q);
  ...
  const totalAceptados = allReportes.filter(
    (r) => r.estado === "Aceptado",
  ).length;
  const totalPendientes = allReportes.filter(
    (r) => r.estado === "Pendiente",
  ).length;
  const totalPorCorregir = allReportes.filter(
    (r) => r.estado === "Por corregir",
  ).length;
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <Text style={{ marginTop: 12, color: C.textMuted, fontSize: 13 }}>
    ...
    >
      ...
      <Row style={{ gap: 12, marginBottom: 22 }}>
        ...
        <StatCard
          ...
          value={String(allReportes.length)}
          icon="file-text"
          iconBg={C.blueLight}
          iconColor={C.blue}
        />
          ...
          {[
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                borderWidth: 1,
                borderColor: filtroTipo === f.id ? C.teal : C.border,
              }}
            >
              <Feather
                name={f.icon}
                size={12}
                color={filtroTipo === f.id ? "white" : C.textMuted}
              />
              <Text
                style={{
                  fontSize: 12,
        ...
        >
          <Row style={{ alignItems: "center", gap: 8 }}>
            ...
            <Text style={{ fontSize: 13, fontWeight: "700", color: C.text }}>
              {totalPendientes} reporte{totalPendientes !== 1 ? "s" : ""}{" "}
              pendiente{totalPendientes !== 1 ? "s" : ""} de revisión
            </Text>
          </Row>
        </View>
      )}
      ...
      <View style={{ gap: 14 }}>
        {residentesFiltrados.map((res) => {
          const aceptados = res.reportes.filter(
            (r) => r.estado === "Aceptado",
          ).length;
          const porCorregir = res.reportes.filter(
            (r) => r.estado === "Por corregir",
          ).length;
          const pendientes = res.reportes.filter(
            (r) => r.estado === "Pendiente",
          ).length;
            ...
            <Card key={res.id} style={{ padding: 0, overflow: "hidden" }}>
              ...
              >
                <Row style={{ alignItems: "center", gap: 14 }}>
                  ...
                  >
                    <Text
                      style={{ fontSize: 14, fontWeight: "800", color: C.teal }}
                    >
                      {res.iniciales}
                    </Text>
                  </View>
                      ...
                      <View style={{ marginTop: 10 }}>
                        ...
                        >
                          <Text style={{ fontSize: 10, color: C.textMuted }}>
                            {aceptados}/{res.reportes.length} reportes aceptados
                          </Text>
                          <Text
                            style={{
                              fontSize: 10,
                        ...
                        >
                          <View
                            style={{
                              height: 6,
                              borderRadius: 3,
                              backgroundColor: pct === 100 ? C.green : C.teal,
                              width: `${pct}%`,
                            }}
                          />
                        </View>
              ...
              {isOpen && (
                ...
                >
                  ...
                  {res.reportes.length === 0 ? (
                    ...
                    >
                      Sin reportes entregados aún.
                    </Text>
                  ) : (
                    <View style={{ gap: 8 }}>
                      {res.reportes.map((rep, ri) => {
                        const st =
                          STATUS_STYLE[rep.estado] || STATUS_STYLE["Pendiente"];
                        return (
                          <Row
                            key={ri}
                            style={{
