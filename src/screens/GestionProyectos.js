...
export default function GestionProyectos() {
  ...
  const handleAsignar = async () => {
    ...
    if (res.ok) {
      ...
      setProyectos((prev) =>
        prev.map((p) =>
          p.id === asignarTarget.id ? { ...p, asesor: nombre } : p,
        ),
      );
              ...
              <Field label="Fase Inicial" C={C}>
                <Row style={{ gap: 8 }}>
                  {FASES.map((f) => {
                    ...
                    const pc = PHASE_COLUMNS.find((c) => c.id === f.id);
                    return (
                      <TouchableOpacity
                        key={f.id}
                        onPress={() =>
                        ...
                        style={{
                          ...
                          borderRadius: 8,
                          borderWidth: 1.5,
                          borderColor: sel ? pc?.color || C.teal : C.border,
                          backgroundColor: sel ? pc?.bg || C.tealLight : C.bg,
                          alignItems: "center",
                        }}
                      >
                        <Text
                          style={{
                            ...
                            fontWeight: "700",
                            textAlign: "center",
                            color: sel ? pc?.color || C.teal : C.textMuted,
                          }}
                        >
                          {f.label}
                        </Text>
