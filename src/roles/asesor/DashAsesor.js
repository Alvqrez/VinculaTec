...
export default function DashAsesor({ onNavigate }) {
  ...
  const datosGraficaReal = useMemo(() => {
    const data = [
      { name: "Aceptados", population: filteredAceptados, color: C.green, legendFontColor: "#6B7280", legendFontSize: 12 },
      { name: "Pendientes", population: filteredPendientes, color: C.amber, legendFontColor: "#6B7280", legendFontSize: 12 },
      { name: "Por corregir", population: filteredPorCorregir, color: C.red, legendFontColor: "#6B7280", legendFontSize: 12 },
    ].filter((d) => d.population > 0);
    if (data.length === 0) return [{ name: "Sin reportes", population: 1, color: "#E5E7EB", legendFontColor: C.text, legendFontSize: 12 }];
    return data;
