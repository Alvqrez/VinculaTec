...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // FIX #4: Emitir SOLO al Jefe de vinculación (room "role_jefe") en lugar
      // de io.emit() que hace broadcast a todos los clientes conectados.
      const io = req.app.get("io");
      if (io) {
        io.to("role_jefe").emit("proyecto_solicitud_avance", {
          proyectoId,
    ...
    try {
      ...
      if (io) {
        ...
        if (usuarioRows.length > 0) {
          io.to(`user_${usuarioRows[0].usuario_id}`).emit(
            "reporte_desbloqueado",
            {
              fase,
              proyectoId,
            },
      ...
      return res.json({
        ok: true,
        mensaje: "Reporte desbloqueado correctamente.",
      });
    } catch (err) {
