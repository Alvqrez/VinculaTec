...
router.get("/asesor/notificaciones", auth, requireRol("asesor"), async (req, res) => {
  try {
...
router.get("/asesor/residentes", auth, requireRol("asesor"), async (req, res) => {
  try {
...
router.post("/", 
  ...
  if (!usuario_id || !tipo_notificacion || !titulo || !mensaje) {
    return res.status(400).json({
      ok: false,
      mensaje: "usuario_id, tipo_notificacion, titulo y mensaje son requeridos.",
    });
  }
  ...
  try {
    ...
    if (proyecto_id) {
      ...
      if (!projRows.length) {
        return res.status(400).json({ ok: false, mensaje: "El proyecto especificado no existe." });
      }
    }
